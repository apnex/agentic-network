...
function loadConfig(directory: string): HubConfig {
  ...
  if (process.env.MCP_HUB_URL) cfg.hubUrl = process.env.MCP_HUB_URL
  if (process.env.HUB_API_TOKEN) cfg.hubToken = process.env.HUB_API_TOKEN
  if (process.env.HUB_PLUGIN_AUTO_PROMPT)
    cfg.autoPrompt = process.env.HUB_PLUGIN_AUTO_PROMPT.toLowerCase() !== "false"
  if (process.env.HUB_PLUGIN_ROLE) cfg.role = process.env.HUB_PLUGIN_ROLE
  return cfg
}
