...
function loadConfig(): HubConfig {
  ...
  const hubUrl = process.env.HUB_URL || fileConfig.hubUrl || "";
  const hubToken = process.env.HUB_TOKEN || fileConfig.hubToken || "";
  const role = process.env.HUB_ROLE || fileConfig.role || "engineer";

  if (!hubUrl || !hubToken) {
    console.error("ERROR: Hub credentials not found. Checked .opencode/hub-config.json and HUB_URL/HUB_TOKEN env vars");
    process.exit(1);
  }
