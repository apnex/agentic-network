...
import { McpAgentClient, loadOrCreateGlobalInstanceId, appendNotification, buildPromptText, makeStdioFatalHalt, createSharedDispatcher, isCacheValid, readCache, writeCache, isEagerWarmupEnabled, parseClaimSessionResponse, formatSessionClaimedLogLine, } from "@ois/network-adapter";
...
function loadConfig() {
    ...
    const labels = parseLabels(process.env.OIS_HUB_LABELS, "OIS_HUB_LABELS env var") ?? fileConfig.labels;
    if (!hubUrl || !hubToken) {
        console.error("ERROR: Hub credentials not found. Checked .ois/adapter-config.json and OIS_HUB_URL/OIS_HUB_TOKEN env vars");
        process.exit(1);
    }
...
function pushChannelNotification(server, event, level) {
    if (!server)
        return;
    ...
    if (data.proposalId)
        meta.proposalId = data.proposalId;
    server
        ...
        .catch((err) => log(`[Channel] Push failed for ${event.event}: ${err}`));
}
function appendActionableLog(event, action) {
    appendNotification({ event: event.event, data: event.data, action }, { logPath: LOG_FILE, mirror: (block) => process.stderr.write(block) });
}
function appendPendingActionLog(item) {
    ...
    appendNotification({ event: item.dispatchType, data: item.payload, action: actionHint }, { logPath: LOG_FILE, mirror: (block) => process.stderr.write(block) });
}
// ── Graceful Shutdown ───────────────────────────────────────────────
let agent = null;
...
async function main() {
    ...
    let rejectIdentityReady;
    const identityReady = new Promise((resolve, reject) => {
    ...
    identityReady.then(() => { identityReadyResolved = true; }).catch(() => { });
    ...
    (async () => {
        try {
            ...
            log(`[Cache] /health fetch failed (non-fatal): ${err.message ?? err}`);
        }
    })();
    agent = new McpAgentClient({
        ...
        handshake: {
            ...
            onPendingTask: (task) => {
                appendNotification({ event: "task_issued", data: task, action: "Pick up with get_task" }, { logPath: LOG_FILE, mirror: (block) => process.stderr.write(block) });
            },
