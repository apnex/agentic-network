...
function getStorage(): Storage {
  if (!sharedStorage) sharedStorage = new Storage();
...
export class GcsStorageProvider implements StorageProviderWithTokenRead {
  ...
  async getWithToken(
    ...
    try {
      ...
      if (generation === undefined || generation === null) {
        throw new StorageProviderError(
          `[gcs] getWithToken missing generation metadata for ${path}`,
        );
      }
      return {
      ...
      if (isNotFound(err)) return null;
      throw new StorageProviderError(
        `[gcs] getWithToken failed for ${path}`,
        err,
      );
    }
  }
  ...
  async list(prefix: string): Promise<string[]> {
    try {
      ...
      throw new StorageProviderError(
        `[gcs] list failed for prefix ${prefix}`,
        err,
      );
    }
  }
  ...
  async createOnly(
    ...
    try {
      ...
      if (isPreconditionFailed(err)) return { ok: false };
      throw new StorageProviderError(
        `[gcs] createOnly failed for ${path}`,
        err,
      );
    }
  }
  ...
  async putIfMatch(
    ...
    if (!Number.isFinite(generation) || generation <= 0) {
      throw new StorageProviderError(
        `[gcs] putIfMatch received invalid token '${ifMatchToken}' — expected stringified GCS generation`,
      );
    }
    const file = this.bucket.file(path);
    try {
      ...
      if (isPreconditionFailed(err)) {
        ...
        try {
          ...
          if (isNotFound(metaErr)) throw new StoragePathNotFoundError(path);
          throw new StorageProviderError(
            `[gcs] putIfMatch precondition-failure recovery failed for ${path}`,
            metaErr,
          );
        }
      }
      ...
      throw new StorageProviderError(
        `[gcs] putIfMatch failed for ${path}`,
        err,
      );
    }
    ...
    try {
      ...
      if (newGen === undefined || newGen === null) {
        throw new StorageProviderError(
          `[gcs] putIfMatch post-write generation missing for ${path}`,
        );
      }
      return { ok: true, newToken: String(newGen) };
    } catch (err: unknown) {
      throw new StorageProviderError(
        `[gcs] putIfMatch post-write metadata fetch failed for ${path}`,
        err,
      );
    }
  }
...
// Structural-subtype sanity check (declaration-only; no instantiation
// because constructor contacts GCS).
type _Check = StorageProvider & GcsStorageProvider;
