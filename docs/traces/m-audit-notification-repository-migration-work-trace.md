...
- ✅ **Verification.** `npm test` (hub): 51 files / 748 passed / 5 skipped (was 50 / 726 / 5 after W8 — delta +1 file / +22 tests / 0 regressions). `npm run build`: clean. `npx tsc --noEmit`: clean. `@apnex/storage-provider` tests: 40 passed (unchanged — no contract delta).
