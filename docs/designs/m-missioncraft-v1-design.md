# M-Missioncraft-V1 — Design v3.3 PENDING-ROUND-4

**Status:** **v3.3 PENDING-ROUND-4** (architect-side; engineer round-3 audit fold on thread-512 — 9 findings: 1 HIGH + 4 MEDIUM + 4 MINOR — all dispositioned; pattern-decay R1=22 → R2=11 → R3=9 convergence-zone start). Composes from **v3.2 PENDING-ROUND-3** at SHA `aa75be4` (engineer round-2 fold-pass; 11 findings dispositioned). v2.5 BILATERAL RATIFIED preserved at SHA `5984334`.
...
2. **Per-repo publish-loop** (atomic across mission's repos; preserves partial state via `mission.publishStatus[<repoName>]` per MEDIUM-R1.9 schema):
   ...
   - **Squash wip-commits** (per `defaults.complete-strategy: squash` operator-config; default squash; opt-in `preserve-history` keeps N commits): **dispatch via `gitEngine.squashCommit(workspace, baseRef, headRef, publishMessage)`** (v3.3 fold per HIGH-R3.1 — pluggable-abstraction discipline). On `UnsupportedOperationError` from 3rd-party engine → engine-internal fallback shell-out to `git merge --squash` + `git commit -m <message>` (per MEDIUM-R1.2 — IsomorphicGit doesn't support squash; native-git-shell-out parallel to §2.6.2 bundle-ops). Capabilities-gated per F13 throws-on-unsupported pattern. Returns squashed-commit-sha. Mark `publishStatus[repo.name] = 'squashed'`.
