# M-Missioncraft-V1 — Design v4.6 PENDING-ROUND-8

**Status:** **v4.6 PENDING-ROUND-8** (architect-side; engineer round-7 audit fold on thread-513 — 3 findings: 0 META + 0 HIGH + 2 MEDIUM + 1 MINOR — over architect's predicted 0-2 envelope by 1; bounded-scope cross-platform precision corrections per substrate-mechanism re-spec catch-net pattern). v4.6 micro-fold commits: R7.1 cross-platform tar mechanism (drop `--mode=0444`; pick two-step extract+chmod for Linux GNU tar / macOS BSD tar uniform behavior); R7.2 chokidar function-form ignored predicate (replaces static glob list; supports DYNAMIC sentinel-existence-check); MINOR-R7.1 Step 3.5 crash-recovery semantic spell-out (idempotent retry; resumes from failed step). Composes from **v4.5 PENDING-ROUND-7** at SHA `9896bf8`. Cumulatively from v4.4 SUBSTANTIAL → v4.4 PARTIAL → v4.3 → v4.2 → v4.1 → v4.0 → v3.6 BILATERAL RATIFIED at SHA `e581a21`.
...
- If fetched ref differs from current → apply via **engine-internal tree-extraction** (v4.5 fold per MEDIUM-R6.2 — replaces v4.0 anchor-prose `git pull --ff-only` which fails against 0444; replaces v4.4 `git checkout-index` mechanism which operated on INDEX not on fetched ref; **v4.6 fold per MEDIUM-R7.1 — cross-platform tar mechanism correction**):
  ```bash
  # Engine creates sentinel-file BEFORE extract (suppresses chokidar Loop A fs-events per Loop A predicate):
  touch <workspace>/missions/<id>/.daemon-tx-active

  # Step 1: chmod working-tree to writable (tar can't overwrite 0444 files)
  chmod -R u+w <reader-workspace>/missions/<id>/<repo>/

  # Step 2: extract tree from fetched ref via git archive | tar
  git archive --format=tar refs/remotes/coord/<repo>/wip/<id> | \
    tar -xC <reader-workspace>/missions/<id>/<repo>/

  # Step 3: chmod recursively to 0444 (TWO-STEP extract+chmod; replaces v4.5 tar --mode=0444 which has GNU/BSD tar cross-platform divergence — GNU tar default on Linux lacks extraction-time --mode option)
  chmod -R 0444 <reader-workspace>/missions/<id>/<repo>/

  # Engine removes sentinel-file AFTER extract complete:
  rm <workspace>/missions/<id>/.daemon-tx-active
  ```
  Cleanest tree-extraction: `git archive` materializes the tree from a fetched ref (no INDEX manipulation; no working-tree git-state); `tar -x` extracts with default umask; `chmod -R 0444` post-extract sets engine-controlled file-mode preserving strict-enforce invariant. **Cross-platform**: works uniformly on Linux (GNU tar) + macOS (BSD/libarchive tar) since `--mode` flag dropped from tar invocation; chmod is POSIX-portable. Brief writable-window inside engine-internal step is suppressed via `.daemon-tx-active` sentinel-file (per Loop A predicate v4.6 fold per MEDIUM-R7.2).
