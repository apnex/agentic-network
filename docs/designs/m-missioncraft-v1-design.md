# M-Missioncraft-V1 — Design v3.4 PENDING-ROUND-5

**Status:** **v3.4 PENDING-ROUND-5** (architect-side; engineer round-4 audit fold on thread-512 — 6 findings: 0 HIGH + 3 MEDIUM + 3 MINOR — all dispositioned; pattern-decay R1=22 → R2=11 → R3=9 → R4=6 convergence-zone deepening). Composes from **v3.3 PENDING-ROUND-4** at SHA `269f226` (engineer round-3 fold-pass; 9 findings dispositioned). v2.5 BILATERAL RATIFIED preserved at SHA `5984334`.
...
interface MissionState {
  ...
  readonly abandonProgress?: 'tick-fired' | 'daemon-killed' | 'locks-released' | 'branches-cleaned' | 'workspace-destroyed' | 'config-purged';
...
`abandon(missionId, message, opts?)` step-sequence under existing mission-lock; persists per-step progress via `mission.abandonProgress: 'tick-fired' | 'daemon-killed' | 'locks-released' | 'branches-cleaned' | 'workspace-destroyed' | 'config-purged'` field for idempotent retry:
...
2. **Terminate daemon-watcher via SIGTERM** (per MEDIUM-R2.1 distinct event); 60s timeout + SIGKILL fallback per MEDIUM-R2.2. Mark `abandonProgress = 'daemon-killed'`.
3. **Persist `lifecycle-state: abandoned` + `abandonMessage`** to config via RMW under mission-lock (atomic-write per MEDIUM-11). abandonMessage immutable post-write per v3.3 fold. **abandonProgress field reflects partial state** until all steps complete.
4. **Release mission-lock + repo-locks** via StorageProvider. Mark `abandonProgress = 'locks-released'`.
5. **Cleanup local mission-branches** in each repo workspace (delete local branch; NO push; NO remote-branch creation; NO PR-open). On per-repo failure (e.g., permission error) → mark `abandonProgress = 'branches-cleaned'` partially; mission-state stays `'in-progress'` (NOT `'abandoned'`); idempotent retry resumes from this step.
...
**Idempotent retry semantics:** if `abandon` fails partway, mission stays `in-progress` with `abandonProgress` reflecting where it failed; abandonMessage already persisted (Step 3 succeeded if reaching later steps). On retry `msn abandon <id> "<msg>"`: engine reads abandonProgress; resumes from failed step; new message-arg IGNORED with operator-warning per v3.3 abandonMessage immutability. Final state advances to `'abandoned'` only after Step 7 (config-purge) OR Step 6 (workspace-destroyed) if `--purge-config` not set; partial-success preserves intermediate state.

**Distinction from complete partial-failure:** publish-flow has per-repo state (publishStatus); abandon has per-step state (abandonProgress) since abandon is sequential not per-repo-parallel. Both use same forensic-history persistence discipline.
...
- **Daemon-watcher lifecycle (v3.1 fold per HIGH-4 + HIGH-5 + HIGH-6 + MEDIUM-R1.4):**
  ...
  - **Dead-pid detection (v3.1 fold per HIGH-6 — immediate detection, not 24h TTL):** every operator-CLI invocation against a `started`/`in-progress` mission performs:
    1. Read lockfile; extract daemon-pid + expiresAt
    2. Check pid alive via `process.kill(pid, 0)` (POSIX kill-with-0; returns 0 if alive; throws ESRCH if dead)
    3. If pid dead → **immediate take-over**: respawn daemon (or operator-prompt "daemon died; respawn? [Y/n]"); reset lockfile pid; mission resumes; logged as "daemon-respawned-after-crash" event
    4. If pid alive but expiresAt > now → respect existing holder; CLI yields per cooperative-coordination
    5. If expiresAt < now → take-over per existing TTL-expired path (link(2) atomic per v2.3 MEDIUM-R3.3)
...
| v3.0 BILATERAL RATIFIED (planned) | TBD | engineer round-N converge-close on thread-512 + architect label-flip + bilateral-commit close | architect-side commit pin + Phase 5 Manifest entry trigger (re-trigger on v3.x close; v2.5 trigger superseded) |
