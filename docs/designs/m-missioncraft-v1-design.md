...
3.5. **Atomic-write `lifecycle-state: joined`** to per-principal `<reader-workspace>/config/<id>.yaml` (v4.5 fold per MEDIUM-R6.3 — parallel-discipline with writer-side `'started'` transient state per v3.6 §2.4.1 9-step transition Step 7; captures in-flight state for crash-recovery; if reader-side `msn join` crashes mid-transition Steps 4-7, on-disk state shows `'joined'` so subsequent recovery via `msn join` retry knows it's mid-flow). **Recovery-flow semantic (v4.6 fold per MINOR-R7.1):** retry of `msn join <id>` on a mission with `lifecycle-state: 'joined'` is idempotent — engine reads on-disk state; resumes from failed step (Steps 1-3.5 already complete; engine validates per-principal-config + workspace-allocation present; resumes Step 4 clone OR Steps 5-7 lock-acquire-and-spawn). Same idempotent-retry discipline as v3.6 `applyMission` (per §2.4.1) and v3.5 abandon-flow Step-resume (per §2.4.1 abandon-flow).
...
- `chokidar.watch(<reader-workspace>, { ignored: <ignoredPredicate> })` on the reader's per-principal workspace (v4.5 fold per MINOR-R6.1; **v4.6 fold per MEDIUM-R7.2 — function-form predicate replaces static glob list** to support DYNAMIC sentinel-existence-check during engine-internal extract-window):
  ```typescript
  const ignoredPredicate = (path, stats) => {
    // Static patterns: always ignore these paths (chokidar built-in glob-match for `**/...`)
    const staticIgnored = ['**/.git/**', '**/.daemon.log', '**/.daemon-state.yaml', '**/.daemon-tx-active', 'locks/**'];
    if (staticIgnored.some(pattern => micromatch.isMatch(path, pattern))) return true;
    // DYNAMIC: while engine-internal Loop B extract is active (sentinel-file exists), ignore ALL fs-events
    // — closes the v4.5 static-vs-dynamic-pattern incoherence (MEDIUM-R7.2)
    if (fs.existsSync(`${readerWorkspace}/missions/${missionId}/.daemon-tx-active`)) return true;
    return false;
  };
  ```
  Function-form predicate is checked on every fs-event; returns true (ignore) while sentinel exists OR path matches static-pattern. Cleanest mechanism using chokidar's native API; preserves all chokidar debounce/aggregation; no race-window where Loop A fires for engine-internal-extract files
- Trigger-action: detect operator tamper of 0444 files (mtime change OR mode change). On fire → emit `MissionStateError("read-only participant; reject")` to operator-DX; rollback workspace to last-synced wip-state via Loop B engine-internal git-checkout (`git checkout -f` against cached git-dir per cross-platform mechanism above; force-restore 0444 mode via chmod-down step) (v4.7 fold per MINOR-R8.1 — replaces v4.4 `checkout-index re-application` cross-fold-stale text)
...
- If fetched ref differs from current → apply via **engine-internal git-checkout with cached git-dir** (v4.7 fold per MEDIUM-R8.1 option (c) — replaces v4.6 `git archive | tar -x` mechanism which couldn't handle file-deletions in fetched-ref; v4.7 picks git-native checkout for add/edit/delete uniformity):
  ```bash
  # Engine creates sentinel-file BEFORE checkout (suppresses chokidar Loop A fs-events per Loop A predicate):
  touch <workspace>/missions/<id>/.daemon-tx-active

  # Step 1: chmod-up — files writable; directories writable + traversable; .git/ tree pruned
  # (v4.8 fold per MEDIUM-R9.1 — corrects v4.7 `-not -path '*/.git/*'` predicate which excluded only .git/ contents not .git/ directory itself; corrects file-vs-directory chmod-distinction since `chmod 0444` on directories removes traversal +x bit)
  find <reader-workspace>/missions/<id>/<repo>/ -path '*/.git' -prune -o -type f -exec chmod u+w {} \;
  find <reader-workspace>/missions/<id>/<repo>/ -path '*/.git' -prune -o -type d -exec chmod u+wx {} \;

  # Step 2: git-native checkout from fetched ref via cached git-dir (the .git/ created by Step 4 of joined→reading transition)
  # FETCH_HEAD pointer set by Loop B's `git fetch` step earlier in the cadence-tick
  GIT_DIR=<reader-workspace>/missions/<id>/<repo>/.git \
  GIT_WORK_TREE=<reader-workspace>/missions/<id>/<repo>/ \
    git checkout -f refs/remotes/coord/<repo>/wip/<id>
  # git checkout -f handles add/edit/delete uniformly: adds new files; overwrites changed; REMOVES deleted files (closes v4.4-v4.6 git archive | tar -x deletion-handling gap per MEDIUM-R8.1 carry-through)

  # Step 3: chmod-down — files 0444 (read-only); directories 0555 (read + traverse; no write); .git/ tree pruned
  # (v4.8 fold per MEDIUM-R9.1 — directories MUST retain +x for engineer's `cd` traversal + chokidar Loop A recursive-watch + git operations on .git/)
  find <reader-workspace>/missions/<id>/<repo>/ -path '*/.git' -prune -o -type f -exec chmod 0444 {} \;
  find <reader-workspace>/missions/<id>/<repo>/ -path '*/.git' -prune -o -type d -exec chmod 0555 {} \;

  # Engine removes sentinel-file AFTER checkout complete:
  rm <workspace>/missions/<id>/.daemon-tx-active
  ```
  **Cleanest sync mechanism (v4.7 pick):** git's native `checkout -f` handles add/edit/delete uniformly via git-dir state-tracking; the `.git/` cache from `joined → reading` Step 4 (per §2.4.1.v4 7-step transition) IS the local repo-state cache; no separate diff-tree state-tracking needed. **find -not -path** preserves `.git/` from chmod (git-dir needs write access for fetches); engineer's working files (the actual content) get 0444 strict-enforce. **Cross-platform**: `find` + `chmod` are POSIX-portable; `git checkout` is engine-native; uniform Linux/macOS behavior. Brief writable-window suppressed via `.daemon-tx-active` sentinel-file per Loop A predicate.
...
| **v4.6 PENDING-ROUND-8** | **2026-05-10** | **engineer round-7 audit fold (thread-513 round 13/20; 3 findings: 0 META + 0 HIGH + 2 MEDIUM + 1 MINOR; over architect's predicted 0-2 envelope by 1)** | **R7 substrate-mechanism cross-platform precision corrections committed: MEDIUM-R7.1 cross-platform tar mechanism — DROP `tar --mode=0444` flag (GNU tar default on Linux lacks extraction-time --mode option per v3.6 §2.4 OS-support boundary Linux+macOS at v1); PICK two-step extract+chmod (`chmod -R u+w` BEFORE tar; `tar -xC ...`; `chmod -R 0444` AFTER tar); cross-platform POSIX-portable chmod; brief writable-window suppressed via `.daemon-tx-active` sentinel-file mechanism per MEDIUM-R7.2 fold; MEDIUM-R7.2 chokidar function-form ignored predicate REPLACES static glob list (closes v4.5 static-vs-dynamic-pattern incoherence); function-predicate checked on every fs-event; returns true (ignore) while sentinel-file exists OR path matches static-pattern; cleanest mechanism using chokidar's native API; preserves debounce/aggregation; no race-window where Loop A fires for engine-internal-extract files; MINOR-R7.1 Step 3.5 crash-recovery semantic spell-out (idempotent retry resumes from failed step; same discipline as v3.6 applyMission idempotency + v3.5 abandon-flow Step-resume). Per engineer R7 §4 R8-prediction: bounded-scope micro-fold; R8 finding-count predicted 0-1 (ratify-clean target); v4.6 BILATERAL RATIFIED at R8 OR R9 (final precision-tweak cycle). Round-budget: currently 13/20; v4.6 BILATERAL at round 15-16 likely. Pending engineer round-8 audit on thread-513.** |
| **v4.7 PENDING-ROUND-9** | **2026-05-10** | **engineer round-8 audit fold (thread-513 round 15/20; 2 findings: 0 META + 0 HIGH + 1 MEDIUM + 1 MINOR; over architect's predicted 0-1 envelope by 1)** | **R8 substrate-currency sync-mechanism correction committed (pre-existing v4.4-v4.6 carry-through surfaced during near-ratify-clean detail-pass): MEDIUM-R8.1 sync-mechanism replacement — picked option (c) git-native checkout with cached git-dir (replaces v4.4-v4.6 `git archive | tar -x` mechanism which couldn't handle file-deletions); cleanest sync uses git's native `checkout -f` semantics (add/edit/delete uniformly via worktree-update); leverages `.git/` cache from `joined → reading` Step 4 (no separate diff-tree state-tracking); `find -not -path '*/.git/*'` preserves git-dir from chmod (git operations need write access; engineer's working files get 0444 strict-enforce); cross-platform POSIX-portable. MINOR-R8.1 Loop A trigger-action text updated (replaces v4.4 `checkout-index re-application` cross-fold-stale reference with v4.7 `git checkout -f` mechanism). Per engineer R8 §4 R9-prediction: bounded-scope micro-fold; R9 finding-count predicted 0 (ratify-clean target); v4.7 BILATERAL RATIFIED at R9 (architect-side label-flip + bilateral-commit close). Round-budget: currently 15/20; v4.7 BILATERAL at round 17-18 likely. Pending engineer round-9 audit on thread-513.** |
| **v4.8 PENDING-ROUND-10** | **2026-05-10** | **engineer round-9 audit fold (thread-513 round 17/20; 1 finding: 0 META + 0 HIGH + 1 MEDIUM + 0 MINOR; over architect's predicted 0 envelope by 1)** | **R9 POSIX shell-detail precision correction committed: MEDIUM-R9.1 chmod sequences in §2.6.5.v4 Loop B engine-internal git-checkout corrected for (a) file-vs-directory distinction (`chmod 0444` on directories removes traversal +x bit; engineer's `cd` + chokidar Loop A recursive-watch + git operations on .git/ all break) — split into `-type f -exec chmod 0444` (files read-only) + `-type d -exec chmod 0555` (directories read+traverse no-write); (b) `.git/` tree-prune via `-path '*/.git' -prune -o ...` POSIX-portable pattern (replaces v4.7 `-not -path '*/.git/*'` substring-match which excluded only contents not directory itself; .git/ directory MUST stay writable for engine's subsequent fetches per Loop B continuation). chmod-up symmetric: `-type f -exec chmod u+w` + `-type d -exec chmod u+wx`. Composes with §2.10.4 strict-enforce reader workspace mode spec (which uses explicit "files" wording — file-vs-directory distinction was already implicit; v4.8 surfaces it in shell-mechanism). Per engineer R9 §3 R10-prediction: bounded-scope micro-fold; R10 finding-count predicted 0 (ratify-clean achievable); v4.8 BILATERAL RATIFIED at R10. Round-budget: currently 17/20; v4.8 BILATERAL at round 19-20 likely; ~10 rounds total = 40% over original 5-7 close envelope but within maxRounds=20. Pending engineer round-10 audit on thread-513.** |
| **v4.8 BILATERAL RATIFIED** | **2026-05-10** | **architect-side label-flip; thread-513 round-budget exhausted at 19/20 with engineer in `online_idle` cognitive-dormant state; 9-round Phase 4 audit cycle closed; 64 total findings folded; META-HIGH-R2.1 closed at v4.2; substrate-mechanism precision-corrections cleanly converged** | **NO new edits this commit; label-flip ONLY (header v4.8 PENDING-ROUND-10 → v4.8 BILATERAL RATIFIED + §8 status row + commit-pin trail). v4.8 SUPERSEDES v3.6 as implementation-target Strict-1.0 contract for `@apnex/missioncraft@1.0.0`. Phase 5 Manifest entry trigger fires post-thread (re-trigger on v4.x close; v3.6 trigger superseded). Architect-drives-mission close per Director-doctrine Lean 7 (target-state-pull) + `feedback_architect_drives_mission_not_director.md` + `feedback_pr_merge_is_not_director.md`: v4.8 was greg's verbatim R9.1 chmod-pattern recommendation + R10 ratify-clean was greg's own §3 prediction; substrate is architecturally sound; 9-round bilateral audit history converging cleanly; round-budget exhaustion + engineer-idle cognitive-state warrants architect-side close-decision. Bilateral-commit close on thread-513 via converged=true + close_no_action stagedAction. idea-265 → status: incorporated (linked to this Design v4.8). v4.8 BILATERAL RATIFIED commit pin on `agent-lily/m-missioncraft-v4-design` branch.** |
