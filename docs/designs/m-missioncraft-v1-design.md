# M-Missioncraft-V1 — Design v3.5 PENDING-ROUND-6

**Status:** **v3.5 PENDING-ROUND-6** (architect-side; engineer round-5 audit fold on thread-512 — 4 findings: 0 HIGH + 2 MEDIUM + 2 MINOR — all dispositioned; pattern-decay R1=22 → R2=11 → R3=9 → R4=6 → R5=4 convergence-zone tail). Composes from **v3.4 PENDING-ROUND-5** at SHA `3a2f7df` (engineer round-4 fold-pass; 6 findings dispositioned). v2.5 BILATERAL RATIFIED preserved at SHA `5984334`.
...
interface MissionState {
  ...
  //   - 'workspace-destroyed': abandon succeeded; config preserved per default --retain not set; lifecycle-state advanced to 'abandoned' atomically at Step 8
  ...
  readonly abandonProgress?: 'tick-fired' | 'daemon-killed' | 'message-persisted' | 'locks-released' | 'branches-cleaned' | 'workspace-destroyed' | 'config-purged';
...
interface ResourceMap {
  mission: {
    ...
    getOpts: undefined;          // v3.1 fold per HIGH-8: undefined (NOT never) — TS-idiomatic; opts-omittable; no call-site ambiguity
    listOpts: undefined;
...
abandonProgress field values: `'tick-fired' | 'daemon-killed' | 'message-persisted' | 'locks-released' | 'branches-cleaned' | 'workspace-destroyed' | 'config-purged'` (v3.5 fold renamed Step 3 progress-marker to `'message-persisted'` to reflect that lifecycle-state does NOT advance at Step 3).
...
4. **Release mission-lock + repo-locks** via StorageProvider. Mark `abandonProgress = 'locks-released'`. (lockfile deleted; abandonInProgress flag implicitly cleared.)
...
6. **Destroy runtime workspace** at `<workspace>/missions/<id>/` (per `--retain` not set). Mark `abandonProgress = 'workspace-destroyed'`.
7. **(if `--purge-config`)** Delete `<id>.yaml` + `.names/<slug>.yaml` symlink (atomic). Mark `abandonProgress = 'config-purged'`.
8. **Atomic-advance lifecycle-state to `'abandoned'`** (v3.5 fold per MEDIUM-R5.1) — ONLY after final cleanup-step succeeds (Step 6 `'workspace-destroyed'` if `--purge-config` not set; Step 7 `'config-purged'` if set). Mission terminal. **Atomicity:** if `--purge-config` set, lifecycle-advance is moot (config deleted at Step 7); the atomic-advance applies to non-purge case where config is preserved with terminal lifecycle-state. Atomic-write of lifecycle-state under mission-lock (re-acquired transiently for Step 8 if Step 4 already released; OR persist within Step 6/7 transaction).

**Idempotent retry semantics:** if `abandon` fails partway, mission stays `'in-progress'` with `abandonProgress` reflecting where it failed; abandonMessage already persisted if reaching Step 4+ (Step 3 succeeded). On retry `msn abandon <id> "<msg>"`: engine reads abandonProgress + abandonRepoStatus; resumes from failed step; new message-arg IGNORED with operator-warning per v3.3 abandonMessage immutability. Final state advances to `'abandoned'` ATOMICALLY at Step 8 — NEVER mid-flow. **Composability with terminal-immutability rule (state-machine table):** mission stays `'in-progress'` throughout abandon-flow → idempotent-retry never hits `MissionStateError("Terminal states are immutable")` because lifecycle-state never advances mid-flow → discipline closes the v3.4 contradiction.

**Distinction from complete partial-failure:** publish-flow has per-repo state (publishStatus) AND atomic terminal lifecycle-advance; abandon-flow now matches BOTH disciplines (abandonRepoStatus per-repo + atomic Step 8 lifecycle-advance) per v3.5 fold. Both flows use same forensic-history persistence discipline + same atomic-terminal-advance discipline.
...
mission:
  ...
  abandon-progress: 'workspace-destroyed'              # v3.4 fold per MEDIUM-R4.1 — abandon-step partial-failure recovery; values: 'tick-fired' | 'daemon-killed' | 'locks-released' | 'branches-cleaned' | 'workspace-destroyed' | 'config-purged'; mission stays 'in-progress' until reaches terminal step; idempotent retry resumes from failed step
...
- **Daemon-watcher lifecycle (v3.1 fold per HIGH-4 + HIGH-5 + HIGH-6 + MEDIUM-R1.4):**
  ...
  - **Dead-pid detection (v3.1 fold per HIGH-6 — immediate detection, not 24h TTL):** every operator-CLI invocation against a `started`/`in-progress` mission performs:
    1. Read lockfile; extract daemon-pid + expiresAt + abandonInProgress (v3.5 fold per MINOR-R5.2)
    2. Check pid alive via `process.kill(pid, 0)` (POSIX kill-with-0; returns 0 if alive; throws ESRCH if dead)
    3. **v3.5 fold per MINOR-R5.2 — abandon-flow concurrent-CLI race guard:** if `lockfile.abandonInProgress === true` AND pid is dead → **DO NOT respawn daemon**; emit `MissionStateError("mission <id> is in abandon-flow; concurrent operation rejected; wait for abandon to complete OR retry msn abandon to resume")` + exit. Closes the abandon-mid-flow concurrent-CLI race where Step 2 (SIGTERM) leaves dead-pid in lockfile until Step 4 (lock-release); without this guard, concurrent CLI invocation between Step 2 and Step 4 would respawn daemon mid-abandon-flow + conflict with cleanup-steps.
    4. If pid dead AND abandonInProgress is not set → **immediate take-over**: respawn daemon (or operator-prompt "daemon died; respawn? [Y/n]"); reset lockfile pid; mission resumes; logged as "daemon-respawned-after-crash" event
    5. If pid alive but expiresAt > now → respect existing holder; CLI yields per cooperative-coordination
    6. If expiresAt < now → take-over per existing TTL-expired path (link(2) atomic per v2.3 MEDIUM-R3.3)
...
- **abandonProgress final-value lifecycle documentation (v3.5 fold per MINOR-R5.1):** added MissionState docstring enumerating final-value semantics (undefined / 'workspace-destroyed' / 'config-purged' / mid-flow values). Closes operator-DX confusion on retrieved-state interpretation; clarifies that `'config-purged'` is transient (never observed in stable terminal state because config + field both deleted at Step 7).
...
| v3.0 BILATERAL RATIFIED (planned) | TBD | engineer round-N converge-close on thread-512 + architect label-flip + bilateral-commit close (predicted R6 ratify-clean OR ~1-2 micro per pattern empirics) | architect-side commit pin + Phase 5 Manifest entry trigger (re-trigger on v3.x close; v2.5 trigger superseded) |
