# M-Missioncraft-V1 — Design v3.1 PENDING-ROUND-2

**Status:** **v3.1 PENDING-ROUND-2** (architect-side; engineer round-1 audit fold on thread-512 — 22 findings: 8 HIGH + 9 MEDIUM + 5 MINOR — all dispositioned; within architect's predicted 15-25 envelope; matches thread-510 R1=24 substrate-shift envelope). Composes from **v3.0 PENDING-BILATERAL** at SHA `afc56e4` (architect's initial 8-refinement reshape). v2.5 BILATERAL RATIFIED preserved at SHA `5984334` on `agent-lily/m-missioncraft-v2-design` branch.
...
interface MissionState {
  ...
  // v3.1 fold per MEDIUM-R1.9 — per-repo publish-state during/after `complete` execution; populated during atomic PR-set publish-flow (per §2.4.1); used for partial-failure idempotent retry. Populated post-`started`; cleared after `lifecycle-state: completed` confirmed all repos `'pr-opened'`.
...
7. **Spawn daemon-watcher (NEW v3.1 fold per HIGH-4):** `child_process.spawn('node', ['<sdk-path>/watcher.js', missionId], { detached: true, stdio: 'ignore' })`; daemon writes its pid to lockfile + extends TTL; daemon enters watch-loop (filesystem-watch via Node fs.watch + debounce-window per §2.6.5). On spawn-failure → rollback: release all locks; mission stays `configured`. Daemon-pid persisted in lockfile for crash-detection (per HIGH-6).
8. **Release transition-pseudolock** (mission-lock RETAINED in lockfile with daemon-pid recorded; daemon holds advisory lock per HIGH-5; cooperative coordination with operator-CLI ops)
9. State = `started`
...
1. **Final cadence-tick** — daemon-watcher's filesystem-watch buffer flushed; pending edits committed to wip-branch (per §2.6.5 daemon-IPC SIGTERM signal-handler grace flush).
2. **Per-repo publish-loop** (atomic across mission's repos; preserves partial state via `mission.publishStatus[<repoName>]` per MEDIUM-R1.9 schema):
   ...
   - **Squash wip-commits** (per `defaults.complete-strategy: squash` operator-config; default squash; opt-in `preserve-history` keeps N commits): **shell-out to `git merge --squash`** (per MEDIUM-R1.2 — IsomorphicGit doesn't support squash per §2.1.4 v0.6 fold; native-git-shell-out parallel to §2.6.2 bundle-ops native-git breach pattern; operator-impact: requires `git` CLI which is already required for §2.6.2). Mark `publishStatus[repo.name] = 'squashed'`.
...
4. **Send SIGTERM to daemon-watcher** via `process.kill(daemonPid, 'SIGTERM')` (per MEDIUM-R1.4 IPC mechanism); daemon's signal-handler does graceful-shutdown (final wip-commit + bundle-snapshot + lockfile release).
...
`abandon` step-sequence MINUS publish-loop (steps 2.0-2.5 only — final tick + cleanup local branches; NO push; NO PR creation). Plus signal daemon-shutdown + release locks + destroy workspace + optional config-purge.
