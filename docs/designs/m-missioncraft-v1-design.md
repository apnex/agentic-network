# M-Missioncraft-V1 — Design v4.3 PENDING-ROUND-4

**Status:** **v4.3 PENDING-ROUND-4** (architect-side; engineer round-3 audit fold on thread-513 — 6 findings: 0 META + 0 HIGH + 4 MEDIUM + 2 MINOR — within architect's predicted 3-6 envelope; META-HIGH-R2.1 CLOSED at v4.2; partial-flatten interim state ACCEPTED with rationale per engineer R3 §6). v4.3 commits R3 architect-picks (M-R3.1 SDK current-principal precedence + M-R3.2 zod superRefine role-based state-validation pick + M-R3.3 separate `.daemon-state.yaml` for lastPushSuccessAt pick + M-R3.4 fold-provenance acknowledgment); substantial inline-flattens for §2.6.5/§2.6.6/§2.4/§2.4.1 + substrate-mechanism fixes (R2.1-R2.4) deferred to v4.4 per partial-flatten discipline. Composes from **v4.2 PENDING-ROUND-3** at SHA `fdcf498`. Cumulatively from v4.1 → v4.0 → v3.6 BILATERAL RATIFIED at SHA `e581a21`.
...
class Missioncraft {
  ...
  // v4.3 fold per MEDIUM-R3.1 — current-principal SDK context-dependency:
  // SDK queries that return per-principal-perspective state (e.g., MissionRepoState.role) need to know "current principal".
  // Resolution precedence (highest to lowest):
  //   1. Explicit per-call override: get('mission', id, { principal: 'lily@apnex' })
  //   2. Missioncraft constructor: new Missioncraft({ principal: 'lily@apnex', ... })
  //   3. Engine-internal IdentityProvider.resolve() composing email field → principal-id
  //   4. Engine-internal MSN_PRINCIPAL_ID env-var (per MEDIUM-R1.7 multi-principal extension)
...
§2.10.3 worked example corrected:

```
Coordination point (single local bare repo; multi-repo via per-repo refs):
  /home/apnex/.missioncraft-coordination/msn-a1b2c3d4.git
    └── refs/heads/design-repo/wip/m-foo    ← lily's daemon pushes here on cadence (one ref per repo)
    └── refs/heads/other-repo/wip/m-foo     (if mission has multiple repos)
                                              greg's daemon pulls each ref on cadence
