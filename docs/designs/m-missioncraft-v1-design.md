# M-Missioncraft-V1 — Design v4.7 PENDING-ROUND-9

**Status:** **v4.7 PENDING-ROUND-9** (architect-side; engineer round-8 audit fold on thread-513 — 2 findings: 0 META + 0 HIGH + 1 MEDIUM + 1 MINOR — over architect's predicted 0-1 envelope by 1; substrate-currency carry-through from v4.4-v4.6 git-archive|tar mechanism file-deletion gap surfaced during near-ratify-clean detail-pass). v4.7 micro-fold commits: R8.1 sync-mechanism replacement — pick option (c) git-native checkout with cached git-dir (handles add/edit/delete uniformly via git's worktree-update; closes file-deletion handling gap; uses `.git/` cache from `joined → reading` Step 4); MINOR-R8.1 Loop A trigger-action cross-fold-stale text fix. Per engineer R8 §4 R9-prediction: ratify-clean target. Composes from **v4.6 PENDING-ROUND-8** at SHA `f0c7ac4`. Cumulatively from v4.5 → v4.4 SUBSTANTIAL → v4.4 PARTIAL → v4.3 → v4.2 → v4.1 → v4.0 → v3.6 BILATERAL RATIFIED at SHA `e581a21`.
...
- If fetched ref differs from current → apply via **engine-internal git-checkout with cached git-dir** (v4.7 fold per MEDIUM-R8.1 option (c) — replaces v4.6 `git archive | tar -x` mechanism which couldn't handle file-deletions in fetched-ref; v4.7 picks git-native checkout for add/edit/delete uniformity):
  ...
  # Step 1: chmod working-tree to writable (git checkout can't overwrite 0444 files; .git/ subdir naturally preserved as not-target)
  find <reader-workspace>/missions/<id>/<repo>/ -not -path '*/.git/*' -exec chmod u+w {} \;
  ...
  # Step 3: chmod recursively to 0444 EXCLUDING .git/ (git-dir needs write access for subsequent fetches; engineer's working files get 0444 strict-enforce per scope-item #2)
  find <reader-workspace>/missions/<id>/<repo>/ -not -path '*/.git/*' -exec chmod 0444 {} \;
