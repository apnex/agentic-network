# M-Missioncraft-V1 — Design v3.2 PENDING-ROUND-3

**Status:** **v3.2 PENDING-ROUND-3** (architect-side; engineer round-2 audit fold on thread-512 — 11 findings: 0 HIGH + 6 MEDIUM + 5 MINOR — all dispositioned; pattern-decay R1=22 → R2=11 cleanly tracks thread-510 envelope). Composes from **v3.1 PENDING-ROUND-2** at SHA `b23ded3` (engineer round-1 fold-pass; 22 findings dispositioned). v2.5 BILATERAL RATIFIED preserved at SHA `5984334`.
...
- **Pid-reuse mitigation via lockfile-startTime (v3.2 fold per round-2 ask 3 — prevents false-alive on dead-pid):** lockfile carries `daemon.startTime` (epoch-ms; recorded at daemon-spawn). HIGH-6 dead-pid detection extends: after `process.kill(pid, 0)` returns 0 (pid alive), engine reads `/proc/<pid>/stat` (Linux) or `ps -p <pid> -o lstart` (macOS POSIX-portable) to verify process-start-time matches lockfile-recorded startTime. If mismatch → pid was reused by unrelated process; treat as dead → respawn. Closes pid-reuse edge case under v1 OS-support boundary Linux+macOS.
