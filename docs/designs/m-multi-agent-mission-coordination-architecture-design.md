# M-Multi-Agent-Mission-Coordination-Architecture — Design v1.2

**Status:** **v1.2** (Director-direct cosmetic-rename fold 2026-05-08-evening — branchcraft → missioncraft + brc → msn references updated throughout per sub-mission #1 v1.1 architectural-refinement reshape; no architectural change to meta-mission catalog). Composes from **v1.1** (Director-direct cosmetic fold 2026-05-08 — symbol-chain format spaces dropped). Composes from **v1.0 BILATERAL RATIFIED** (thread-507 round-5 engineer converged-close + round-6 architect label-flip + bilateral-commit close). v0.1 → v0.2 → v1.0 PENDING-BILATERAL → v1.0 BILATERAL RATIFIED → v1.1 cosmetic fold → v1.2 cosmetic-rename fold (this version). Round-budget closed at 6/12 for v1.0 audit-cycle — first-canonical mission-of-missions audit-cadence validated. See §8 Status for full version-trajectory.
...
- First-canonical **multi-repo mission** shape — missioncraft sub-mission lives in `github.com/apnex/missioncraft` (new sovereign repo) while idea/mission/audit entities live in `github.com/apnex-org/agentic-network`
...
**Architectural framing:** *"Build missioncraft v1 as architecturally-complete, OSS-from-day-1, category-contribution standalone tool at `github.com/apnex/missioncraft`. Build citation-validator second. Migrate OIS components 1:1 to repos via per-component sequential sub-missions, leaf-most components first, hub-repo last. Cross-repo breaking changes use lockstep-coordination + semver-major-bump signaling — clean-break posture; semver IS the spec; no deprecation-runway. Mission-of-missions deliverable IS this catalog."*

**Tele anchors (ratified):**
- **tele-3 Sovereign Composition** PRIMARY — full pluggable architecture from missioncraft v1; Law-of-One per pluggable interface; current OIS package boundaries preserved as sovereign-module boundaries during migration
- **tele-2 Isomorphic Specification** PRIMARY — missioncraft v1 API IS the spec; bit-perfect contracts published from day 1; semver IS the cross-repo truth-channel; symbol-chain citations enforce spec-as-runtime
...
- (R1) standalone-repo from start → missioncraft sub-mission lives in github.com/apnex/missioncraft (NEW sovereign repo)
...
|---|---|---|---|---|---|---|
| 1 | M-Missioncraft-V1 | substrate-introduction | NEW: `github.com/apnex/missioncraft` | Sovereign component: 5 pluggables (`IdentityProvider` / `ApprovalPolicy` / `StorageProvider` / `GitEngine` / `RemoteProvider`); 2 personas (standalone-CLI `msn` + library-SDK); IsomorphicGit primary; gh CLI GitHubRemoteProvider | none (foundational) | tele-3, tele-2 |
| 2 | M-Citation-Validator-Tooling | substrate-introduction | missioncraft repo (subcommand `msn citations validate`) per F1 | Cross-repo citation resolver + PR-time validator gate; symbol-chain format `<component>><file-path>><class.method|function|symbol|type>` (v1.1 cosmetic: no spaces around `>` separator; Director-direct fold 2026-05-08); version-aware resolution | #1 | tele-2, tele-12 |
...
**Sub-mission count rationale:** per-component-1-mission (8 component-extracts + missioncraft + citation-validator + agentic-network-substrate-purge) = 11 sub-missions. Q2=c phased-incremental philosophy + corrected dependency-graph confirms no natural batching boundary.
...
### §2.2 Missioncraft v1 internal architecture (sub-mission #1 detail)

This section sketches the v1 architectural shape — Phase 4 Design of M-Missioncraft-V1 (a separate sub-mission) will concretize. Here we lock the umbrella scope.
...
**v1 default-stack composition:** `LocalGitConfigIdentity` + `TrustAllPolicy` + `LocalFilesystemStorage` + `IsomorphicGitEngine` + (no RemoteProvider; pure-git mode). Operator opts in to GitHub features via `msn remote add github --provider=gh-cli`.

#### §2.2.2 Personas (2 v1-shipped per Q1=b)

- **Standalone-CLI persona:** `msn <verb>` invocation surface. Verbs (v1 minimum): `init`, `clone`, `branch`, `commit`, `push`, `pull`, `merge`, `mission start`, `mission complete`, `citations validate` (per #2 sub-mission). Operator-friendly; `msn --help` documented.
- **Library-SDK persona:** `import { Missioncraft } from '@apnex/missioncraft'` consumption. Programmatic API mirrors CLI verbs. TypeScript-first; ships `.d.ts`.
- **OIS-orchestrated persona (POST-V1; per F8 architect-recommendation):** OIS-side integration code consumes missioncraft library-SDK. NOT missioncraft-internal. Built ON TOP via OIS-side adapter (post-v1 sub-mission, OR included in M-Component-Repo-Extract-Hub when hub gains missioncraft-orchestration capability).
...
- **Single-writer-per-mission lock:** acquired at `msn mission start`; released at `msn mission complete` (or `msn mission abandon`); enforced by `ApprovalPolicy` if configured
...
- **Both surfaces:** CLI flag (`msn merge --auto`) for ad-hoc; mission-config-driven (declarative `auto-merge: true` in mission manifest) for governance-scope missions
...
Mechanism PROBE-flagged for engineer-architect-bilateral resolution. Candidate options to consider in M-Missioncraft-V1 Design:
...
- `missioncraft>src/engine/isomorphic-git-engine.ts>IsomorphicGitEngine.commit`
...
- **Architect-recommendation:** missioncraft CLI subcommand (`msn citations validate <pr-diff-or-file>`) — single sovereign tool surface; reuses missioncraft's `IsomorphicGit` + `RemoteProvider` for cross-repo resolution; avoids second sovereign-tool maintenance surface
- **Engineer round-1 audit ask:** challenge if scope balloons beyond missioncraft natural-fit (e.g., LSP-server-shaped resolver benefits from separate ops-repo)
...
6. **Missioncraft mission orchestration:** use missioncraft to coordinate cross-repo branch + commit + push (this is the FIRST validation that missioncraft v1 + citation-validator work in production-extraction shape)
...
13. **Mission complete:** missioncraft `mission complete` releases lock; sub-mission Phase 9 close
...
- **One workspace per (mission, repo) tuple:** a multi-repo mission spans multiple workspaces; missioncraft coordinates
...
Ops-repos (e.g., monitoring scripts; infra-as-code; operator dashboards) are the ONLY long-lived workspace category per Director ratification 2026-05-08. Missioncraft supports `--ops` flag at workspace creation to mark long-lived; mission-lock semantics relaxed for ops-repos (no mission-bound lifecycle).

#### §2.5.3 GitHub org boundary discipline (tele-3 mechanism)

- **`github.com/apnex/missioncraft`** (sovereign personal namespace) — missioncraft is category-tool; OIS happens to use it
...
This namespace separation operationalizes tele-3 Sovereign Composition at GitHub-infrastructure-level: missioncraft cannot accidentally couple to OIS-org-internal substrate because they're distinct GitHub orgs.
...
|---|---|---|---|---|
| F1 | Citation-validator host: missioncraft CLI subcommand vs separate ops-repo tool | MEDIUM | RATIFIED — msn subcommand (§2.3.3) — single sovereign tool surface; reuses IsomorphicGit + RemoteProvider; LSP-server-shape can come post-v1 if scope balloons. Engineer round-1 concur. |
...
| F5 | Workspace-state durability mechanism | PROBE | DEFER — to M-Missioncraft-V1 Phase 3 Survey (Director-intent question on failure-mode protection: process-crash vs disk-failure vs network-partition-during-push). Engineer round-1 concur. |
...
| F8 | OIS-orchestrated persona scope: missioncraft v1 internal OR post-v1 sub-mission | **CRITICAL** | **RATIFIED — post-v1 sub-mission** (per §2.2.2). Engineer concur: library-SDK persona (Q1=b) covers migration-playbook use case; OIS-internal coupling stays out of missioncraft v1; category-tool-shape preserved. |
| F9 | Sub-mission count rationale (11 vs 10 vs ~15) | MEDIUM | RATIFIED — 11 per §2.1 (1 missioncraft + 1 citation-validator + 8 component-extracts + 1 substrate-purge). Engineer concur. |
...
| F12 | Hub-orchestration mechanism for cross-repo lockstep PR-fan-out | PROBE | RATIFIED — missioncraft-CLI-driven (`msn cross-repo-pr fan-out --target=v1.0.0`); concrete design defers to M-Missioncraft-V1 Phase 4. Engineer round-1 concur architect-lean: same-tool-orchestrates-both pattern; Hub-MCP-tool-driven is heavier-weight + couples to OIS-internal. |
...
|---|---|---|---|
| #1 M-Missioncraft-V1 | tele-3, tele-2 | tele-11, tele-7 | Sovereign-composition (5 pluggables) + spec-as-runtime (v1 API IS spec) |
...
### §6.1 Composition risk: missioncraft v1 + citation-validator dual-substrate ship
...
### §6.4 Open question: missioncraft v1 ship timeline (calendar vs deliverable-sized)
...
**v0.1 framing was incorrect** — conflated GitHub-org-name with npm-scope. Current npm scope `@apnex/*` (e.g., `@apnex/storage-provider`, `@apnex/network-adapter`) is RETAINED through migration; not changing. The migration variable is **GitHub repo location** (`github.com/apnex-org/<component>` for OIS components; `github.com/apnex/missioncraft` for sovereign-tool) + version (semver-major-bump per Q5=ac signaling).
...
**v0.2 ratification:** **(b) just-in-time** — Phase 5 Manifest creates only sub-mission #1 (M-Missioncraft-V1) at meta-mission Phase 5; subsequent sub-missions created at execution-time-window (when their predecessor is closing OR their dependency-graph clears).
...
- **Candidates-surfaced (v0.2 augmented per engineer round-1 calibration #62 audit-rubric application):**
  ...
  - **NEW v0.2 deferred-surfaces enumerated for transparency** (each is a calibration #62 candidate; tracking explicitly to defeat substrate-described-but-not-substrate-tested pattern):
    - Workspace-state durability mechanism (§2.2.5 PROBE; defer to M-Missioncraft-V1 Phase 3 Survey)
    - Hub-orchestration mechanism for cross-repo lockstep PR-fan-out (F12 PROBE; defer to M-Missioncraft-V1 Phase 4)
...
| v1.0 BILATERAL RATIFIED | 2026-05-08 | engineer round-3 converge-close (thread-507 round 5/12) + architect label-flip + bilateral-commit close (round 6/12) | label-flip only — no substantive content change from v1.0 PENDING-BILATERAL @ SHA `1ca56da`. Pushed at SHA `4112fc1`. Phase 5 Manifest unblocked: creates sub-mission #1 (M-Missioncraft-V1) Hub entity at execution-time-window per §6.6 (b) just-in-time discipline; downstream sub-missions per topological order |
| v1.1 | 2026-05-08 | Director-direct cosmetic fold post bilateral-ratify | symbol-chain format spaces dropped around `>` separator at §2.1 row 2 + §2.3.1 examples (v1.0: `<component> > <file-path> > <symbol>` → v1.1: `<component>><file-path>><symbol>`). No architectural change; cosmetic literal-form only. M-Branchcraft-V1 (now M-Missioncraft-V1) Phase 5 launch authorization composed with this fold; pushed at SHA `7c859d4` |
| **v1.2** | **2026-05-08-evening** | **Director-direct cosmetic-rename fold post sub-mission #1 v1.1 architectural-refinement** | **this version; branchcraft → missioncraft + brc → msn references updated throughout (§2.1 row 1 + §2.2 + §2.3 + §6.5 + §3 F23 row + sub-mission #1 references in catalog table + Survey envelope §0 + §3 + §1.Q3). NO architectural change to meta-mission catalog; sub-mission #1 internal architecture refined via separate v1.1 reshape on `agent-lily/m-missioncraft-v1-design` branch. M-Missioncraft-V1 entity (idea-263) retitled via `update_idea`; old M-Branchcraft-V1 design at v1.0 BILATERAL RATIFIED preserved at SHA `7fb1643` on `agent-lily/m-branchcraft-v1-survey` branch as historical artifact** |
