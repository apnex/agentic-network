...
| T1 | `@apnex/repo-event-bridge` sovereign-package contract | ✅ Merged | `906f6bf` | #52 | +45 in-package (translator unit + conformance + CreateMessageSink end-to-end); 0 hub-side |
...
- Package test baseline: 0 → 45 (T1) → 106 (T2) → 106 (T3) → 106 (T4) → **106 (T5)** = **+106 tests in `@apnex/repo-event-bridge` across mission-52; 0 regressions throughout**.
...
| `@apnex/repo-event-bridge` | — | 45 | 106 | 106 | 106 | 106 |
...
**Cross-package verification:** `@apnex/storage-provider` contract surface unchanged. ADR-024 §2 not amended. `@apnex/repo-event-bridge` is the new sovereign package; ADR-pending (architect call on whether ADR-026 ratifies the EventSource contract or whether the package's README + this audit suffice). Cross-package failures match pre-existing bug-32 pattern.
...
|---|---|---|---|
| 1 | `@apnex/repo-event-bridge` sovereign package ratified with EventSource async-iterator interface + capability flags + health() probe | ✅ MET | T1 (PR #52) — `EventSource`, `EventSourceCapabilities`, `EventSourceHealth` types in `packages/repo-event-bridge/src/event-source.ts`; 5 capability axes; sync health() snapshot |
...
| 8 | Per-repo cursor + bounded LRU dedupe via `@apnex/storage-provider` | ✅ MET | T2 — `CursorStore` class wraps StorageProvider; `putIfMatch` atomic cursor; touch-on-access LRU dedupe (capacity 1000 default); Hub-restart resumption test verifies persistence across PollSource re-instantiation |
...
- `@apnex/cognitive-layer`: 172/172 throughout.
- `@apnex/network-adapter`: 30 failures match pre-existing bug-32 pattern.
- `@apnex/repo-event-bridge` (new): 106/106 throughout T1-T5.
...
Mission-52 ratifies `@apnex/repo-event-bridge` as the fifth sovereign package alongside `@apnex/network-adapter`, `@apnex/cognitive-layer`, `@apnex/storage-provider`, and `@apnex/message-primitive` (in-Hub but ratified per ADR-025). The sovereign-package shape (separate `package.json` + `tsconfig.json` + `vitest.config.ts` + dedicated test surface + composable contract) is now load-tested across 5 instances. Per-package onboarding cost is dropping — T1's package scaffolding + contract surface shipped in ~50 min; T2 added concrete impl in ~45 min.
...
2. **prepare-script** (round 2) — `"prepare": "tsc"` in the package's `package.json` so consumers auto-build on install. Failed because tsc runs before the package's own `node_modules/@apnex/storage-provider` is reachable in the install order — chicken-and-egg.
...
- **Sibling sovereign-package precedents:**
  - `@apnex/network-adapter` (mission-39 era; pre-ADR)
  - `@apnex/cognitive-layer` (ADR-018)
  - `@apnex/storage-provider` (mission-47; ADR-024)
  - `@apnex/message-primitive` (in-Hub; mission-51; ADR-025)
- **Companion ADRs:**
...
- **idea-186 sunset cleanup checklist** (post-workspaces):
  ...
  7. Optionally remove `peerDependencies` declaration on `@apnex/storage-provider` in `packages/repo-event-bridge/package.json` (workspaces resolve the dep natively without peer-hoist)
...
mission-52 (M-Repo-Event-Bridge) closes the repo-event ingestion arc opened by idea-191 + ratified at thread-312 round 2. The mission ships sovereign-package #5 (`@apnex/repo-event-bridge`) as a sibling to network-adapter / cognitive-layer / storage-provider / message-primitive. The package contributes the EventSource async-iterator interface + 8-subkind GH-event translator with `unknown` graceful-degrade + CreateMessageSink stub mapping RepoEvent → mission-51 W6's `create_message` MCP verb + a fully-implemented PollSource concrete source (PAT auth + 3-layer 429 handling + cursor + dedupe via `@apnex/storage-provider` eating own dogfood) + a WebhookSource design doc capturing cloud-Hub future architecture without runtime risk.
